...
import { GripVertical, Star, ArrowDownAZ, BarChart3, Calendar, ChartNoAxesCombined, ChevronDown, Columns3, Copy, Download, FilePenLine, Filter, GalleryHorizontal, LayoutGrid, List, Map, MoreHorizontal, Plus, Search, Table2, Trash2, X } from 'lucide-react';
...
const layouts = [{ id: 'table', name: 'Table', ar: 'جدول', Icon: Table2 }, { id: 'board', name: 'Board', ar: 'لوحة', Icon: Columns3 }, { id: 'gallery', name: 'Gallery', ar: 'معرض', Icon: GalleryHorizontal }, { id: 'list', name: 'List', ar: 'قائمة', Icon: List }, { id: 'chart', name: 'Chart', ar: 'مخطط', Icon: ChartNoAxesCombined }, { id: 'timeline', name: 'Timeline', ar: 'خط زمني', Icon: BarChart3 }, { id: 'map', name: 'Map', ar: 'خريطة', Icon: Map }, { id: 'calendar', name: 'Calendar', ar: 'تقويم', Icon: Calendar }, { id: 'form', name: 'Form', ar: 'نموذج', Icon: FilePenLine }] as const;
type Props = {
    ...
    {panel && <div className={`notion-db-menu ${panel === 'new-view' ? 'notion-db-menu--start' : ''}`} role="dialog" aria-label={ar ? 'إعدادات قاعدة البيانات' : 'Database options'}>
      ...
      {panel === 'templates' && <>
        ...
        {props.templates.map((template) => {
          const isDefault = activeView?.layoutConfig.defaultTemplateId === template.id;
          return <div className="template-menu-row" key={template.id} onDragOver={event => { if (dragTemplate) event.preventDefault(); }} onDrop={event => { event.preventDefault(); if (dragTemplate) void reorderTemplate(dragTemplate, template.id); setDragTemplate(undefined); }}>
            <button type="button" className="template-drag" aria-label={(ar ? 'إعادة ترتيب ' : 'Reorder ') + template.name} draggable onDragStart={event => { setDragTemplate(template.id); event.dataTransfer.setData('text/plain', template.id); }} onDragEnd={() => setDragTemplate(undefined)} onKeyDown={event => { if (event.altKey && ['ArrowUp', 'ArrowDown'].includes(event.key)) { event.preventDefault(); const index = props.templates.findIndex(t => t.id === template.id); const target = props.templates[index + (event.key === 'ArrowUp' ? -1 : 1)]; if (target) void reorderTemplate(template.id, target.id); } }}><GripVertical size={13} /></button>
            <button type="button" className="template-menu-open" onClick={() => action(() => props.onCreate(template.id))}><PageIconRenderer icon={template.icon ?? 'lucide:FileText'} size={16} /><span className="template-menu-name">{template.name}</span>{isDefault && <small>{ar ? 'افتراضي' : 'Default'}</small>}</button>
            <button type="button" className="template-menu-more" aria-label={(ar ? 'خيارات ' : 'Options for ') + template.name} aria-expanded={templateMenu === template.id} onClick={() => setTemplateMenu(templateMenu === template.id ? null : template.id)}><MoreHorizontal size={15} /></button>
            {templateMenu === template.id && <div className="template-menu-actions" role="menu">
              <button type="button" role="menuitem" onClick={() => { setTemplateMenu(null); setPanel(null); setTemplateEditor(template); }}><FilePenLine size={14} />{ar ? 'تحرير' : 'Edit'}</button>
              {activeView && <button type="button" role="menuitem" onClick={() => { void props.onUpdateView(activeView.id, { layoutConfig: { ...activeView.layoutConfig, defaultTemplateId: isDefault ? null : template.id } }); setTemplateMenu(null); }}><Star size={14} />{isDefault ? (ar ? 'إلغاء الافتراضي' : 'Remove default') : (ar ? 'تعيين كافتراضي' : 'Set as default')}</button>}
              <button type="button" role="menuitem" disabled={busy} onClick={() => { setBusy(true); void window.maxApi.workspace.archiveRecordTemplate(template.id).then((result) => { if (!result.ok) setTemplateError(result.error.message); else { window.dispatchEvent(new Event('max:workspace-changed')); setTemplateMenu(null); } }).catch(() => setTemplateError(ar ? 'تعذر أرشفة القالب.' : 'Could not archive template.')).finally(() => setBusy(false)); }}><Trash2 size={14} />{ar ? 'أرشفة' : 'Archive'}</button>
            </div>}
          </div>;
        })}
