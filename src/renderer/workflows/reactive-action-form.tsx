 ...
 *
 * Pressing Run always answers. Earlier it was simply disabled whenever a
 * preview was in flight or a rule objected, so a sale that could not go through
 * looked identical to one nobody had pressed: nothing moved and nothing said
...
export function ReactiveActionForm({ workflow, locale, onCompleted, onBusy }: { workflow: WorkspaceWorkflow; locale: Locale; onCompleted: () => void; onBusy: (busy: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { void window.maxApi.workspace.evaluateWorkflow({ workflowId: workflow.id, inputs: values, overrides, evaluationToken: previewToken.current }).then(result => {
      ...
      if (!result.ok) { setError(result.error.message); setEvaluation(undefined); setQueued(false); return; }
    ...
    }).catch(() => { if (active) { setError(ar ? 'تعذر تحديث المعاينة.' : 'Could not update preview.'); setQueued(false); } }); }, 120);
  ...
  const blocks = evaluation?.messages.filter(m => m.severity === 'BLOCK') ?? [];
  ...
  const attempt = (confirmed = false) => {
    if (lock.current || running) return;
    // Still previewing: hold the press rather than dropping it.
    if (pending || !evaluation) { setQueued(true); setError(''); return; }
    setQueued(false);
    const gaps = missingRequiredInputs(workflow.inputSchema.fields, values, evaluation);
    if (gaps.length) { setMissing(gaps.map(gap => gap.address)); setError(`${ar ? 'أكمل أولاً: ' : 'Fill in first: '}${listMissing(gaps, ar ? 'ar' : 'en')}`); return; }
    setMissing([]);
    if (blocked) { setError(blocks.map(block => block.message).join(' · ')); return; }
