...
function viewFromRow(row: ViewRow): WorkspaceView {
  return {
    ...
    filterAst: row.filter_ast_json ? JSON.parse(row.filter_ast_json) : null,
    group: row.group_json ? JSON.parse(row.group_json) : null,
    id: row.id,
    layout: row.layout,
    ...
    propertyState: JSON.parse(row.property_state_json || '{"columns":[]}'),
    sorts: JSON.parse(row.sort_json || '[]'),
